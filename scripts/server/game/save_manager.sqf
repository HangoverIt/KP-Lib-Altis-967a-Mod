    ...
    if (((_saveData select 0) select 0) isEqualType 0) then {
        {
            ...
            {
                [_x, [_spawnPos, _grp] select (_forEachIndex > 0), _grp, "PRIVATE", 0, false] call KPLIB_fnc_createManagedUnit;
            } forEach _units;
        } forEach _aiGroups;
    } else {
        ...
        private _unit = objNull;
        {
            _grp = createGroup [GRLIB_side_friendly, true];
            {
                ...
                _dir = _x select 2;
                _unit = [(_x select 0), _pos, _grp, "PRIVATE", 0, false] call KPLIB_fnc_createManagedUnit;
                _unit setDir _dir;
